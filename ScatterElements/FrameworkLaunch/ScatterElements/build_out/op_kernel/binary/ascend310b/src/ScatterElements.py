...
def _build_args(var_in__, indices_in__, updates_in__, axis, reduce):
    __inputs__ = []
...
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_STR, para_check.KERNEL_NAME)
def scatter_elements(var_in__, indices_in__, updates_in__, axis=0, reduce="assign", kernel_name="scatter_elements", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, indices_in__, updates_in__, axis, reduce)
    ...
    print("start compile Ascend C operator ScatterElements. kernel name is " + kernel_name)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[var_in__, indices_in__, updates_in__], origin_outputs = [],\
                param_type_dynamic = False, mc2_ctx = [], param_type_list = ['required', 'required', 'required'], init_value_list = [],\
                output_shape_depend_on_compute = [])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(var_in__, indices_in__, updates_in__, axis=0, reduce="assign", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, indices_in__, updates_in__, axis, reduce)
    result = check_op_cap("op_select_format", "ScatterElements", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(var_in__, indices_in__, updates_in__, axis=0, reduce="assign", impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(var_in__, indices_in__, updates_in__, axis, reduce)
