...
def _build_args(boxes_in__, scores_in__, max_output_boxes_per_class_in__, iou_threshold_in__, score_threshold_in__, selected_indices_out_, center_point_box):
    __inputs__ = []
    for arg in [boxes_in__, scores_in__, max_output_boxes_per_class_in__, iou_threshold_in__, score_threshold_in__]:
        if arg != None:
            ...
            __inputs__.append(arg)
    __outputs__ = []
    for arg in [selected_indices_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
            ...
            __outputs__.append(arg)
    __attrs__ = []
    if center_point_box != None:
        attr = {}
        attr["name"] = "center_point_box"
        attr["dtype"] = "int"
        attr["value"] = center_point_box
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("NonMaxSuppression", trans_bool_to_s8=False)
...
def non_max_suppression(boxes_in__, scores_in__, max_output_boxes_per_class_in__, iou_threshold_in__, score_threshold_in__, selected_indices_out_, center_point_box=0, kernel_name="non_max_suppression", impl_mode=""):
    # do ascendc build step
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(boxes_in__, scores_in__, max_output_boxes_per_class_in__, iou_threshold_in__, score_threshold_in__, selected_indices_out_, center_point_box)
    ...
    origin_func_name = "non_max_suppression"
    ascendc_src_dir_ex = "non_max_suppression"
    ascendc_src_dir = "non_max_suppression"
    ascendc_src_file = "non_max_suppression.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator NonMaxSuppression. kernel name is " + kernel_name)
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[boxes_in__, scores_in__, max_output_boxes_per_class_in__, iou_threshold_in__, score_threshold_in__], origin_outputs = [selected_indices_out_],\
...
def op_select_format(boxes_in__, scores_in__, max_output_boxes_per_class_in__, iou_threshold_in__, score_threshold_in__, selected_indices_out_, center_point_box=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(boxes_in__, scores_in__, max_output_boxes_per_class_in__, iou_threshold_in__, score_threshold_in__, selected_indices_out_, center_point_box)
    result = check_op_cap("op_select_format", "NonMaxSuppression", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(boxes_in__, scores_in__, max_output_boxes_per_class_in__, iou_threshold_in__, score_threshold_in__, selected_indices_out_, center_point_box=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(boxes_in__, scores_in__, max_output_boxes_per_class_in__, iou_threshold_in__, score_threshold_in__, selected_indices_out_, center_point_box)
    result = check_op_cap("get_op_specific_info", "NonMaxSuppression", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
