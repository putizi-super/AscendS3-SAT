...
[[maybe_unused]] uint32_t NNOPBASE_NonMaxSuppression = 0U;
} // namespace
    ...
    aclOpExecutor **executor)
{
    ...
    static NnopbaseDfxId tilingId = {0x60000, "aclnnNonMaxSuppressionTiling", false};
    void *nnopExecutor;
    static void *executorSpace = NULL;
    const char *opType = "NonMaxSuppression";
    char inputDesc[] = {1, 1, 1, 1, 1};
    char outputDesc[] = {1};
    char attrDesc[] = {0};

    NNOPBASE_ASSERT_NOTNULL_RETVAL(boxes);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(scores);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(maxOutputBoxesPerClass);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(iouThreshold);
    NNOPBASE_ASSERT_NOTNULL_RETVAL(scoreThreshold);
    ...
    *executor = reinterpret_cast<aclOpExecutor *>(nnopExecutor);
    ...
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, boxes, 0));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, scores, 1));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, maxOutputBoxesPerClass, 2));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, iouThreshold, 3));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddInput(*executor, scoreThreshold, 4));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddAttrWithDtype(*executor, static_cast<void*>(&centerPointBoxOptional), sizeof(int64_t), 0, kNnopbaseInt));
    NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddOutput(*executor, out, 0));
    if (NnopbaseAddParamName != NULL) {
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 0, "boxes", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 1, "scores", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 2, "maxOutputBoxesPerClass", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 3, "iouThreshold", true));
        NNOPBASE_ASSERT_OK_RETVAL(NnopbaseAddParamName(*executor, 4, "scoreThreshold", true));
