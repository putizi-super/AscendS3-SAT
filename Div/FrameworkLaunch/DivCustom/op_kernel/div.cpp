 ...
 *
 * Function : z = x / y
 * This sample is a very basic sample that implements vector div on Ascend plaform.
 */
...
template<typename TYPE_X, typename TYPE_Y, typename TYPE_Z> class KernelDiv {
    using T = TYPE_X;
public:
...
private:
    ...
    __aicore__ inline void Compute(int32_t progress)
    {
      ...
      if constexpr (std::is_same_v<T, int8_t>) {
        ...
        AscendC::Cast(p1.ReinterpretCast<int16_t>(), p2, AscendC::RoundMode::CAST_TRUNC, this->processDataNum);
        ...
        AscendC::Cast(p2, p1.ReinterpretCast<int16_t>(), AscendC::RoundMode::CAST_TRUNC, this->processDataNum);
        AscendC::Cast(zLocal, p2, AscendC::RoundMode::CAST_TRUNC, this->processDataNum);
      }
      else if constexpr (std::is_same_v<T, int32_t>) {
...
extern "C" __global__ __aicore__ void div(GM_ADDR x, GM_ADDR y, GM_ADDR z, GM_ADDR workspace, GM_ADDR tiling)
{
...
void div_do(uint32_t blockDim, void* l2ctrl, void* stream, uint8_t* x, uint8_t* y, uint8_t* z,
    uint8_t* workspace, uint8_t* tiling)
{
    div<<<blockDim, l2ctrl, stream>>>(x, y, z, workspace, tiling);
}
#endif
