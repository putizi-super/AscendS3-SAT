...
bool OpRunner::RunOp()
{
	...
	auto ret = aclnnDivCustomGetWorkspaceSize(inputTensor_[0], inputTensor_[1], outputTensor_[0],
	...
	INFO_LOG("Execute aclnnDivCustomGetWorkspaceSize success, workspace size %lu", workspaceSize);
    ...
    ret = aclnnDivCustom(workspace, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS) {
        (void)aclrtDestroyStream(stream);
        ERROR_LOG("Execute Operator failed. error code is %d", static_cast<int32_t>(ret));
        return false;
    }
	INFO_LOG("Execute aclnnDivCustom success");
