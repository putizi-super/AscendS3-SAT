...
def verify_result(real_result, golden):
    ...
    deno = np.maximum(np.abs(real_result), np.abs(golden)).astype(np.float32)  # 获取最大值并组成新数组 
    ...
    if not result_rtol.all() and not result_atol.all():
        if np.sum(result_rtol == False) > real_result.size * loss and np.sum(result_atol == False) > real_result.size * loss: # 误差超出预期时返回打印错误，返回对比失败
            print(real_result[np.where(result_atol == False)[:5]])
            print(golden[np.where(result_atol == False)[:5]])
            print(np.where(result_rtol == False))
