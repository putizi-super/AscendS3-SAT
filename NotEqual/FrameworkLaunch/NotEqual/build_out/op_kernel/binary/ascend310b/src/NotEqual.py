...
@tbe_register.register_operator("NotEqual")
...
def not_equal(x1, x2, y, kernel_name="not_equal", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x1, x2, y)
    ...
    print("start compile Ascend C operator NotEqual. kernel name is not_equal")
    op_type = "NotEqual"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x1, x2], origin_outputs = [y])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x1, x2, y, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1, x2, y)
    result = check_op_cap("op_select_format", "NotEqual", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x1, x2, y, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x1, x2, y)
